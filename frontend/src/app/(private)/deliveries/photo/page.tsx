...
export default function PhotoDeliveryPage() {
  ...
  const [cleanedOcrText, setCleanedOcrText] = useState("");
  ...
  const handleTransform = async () => {
    ...
    if (!imageBase64) {
      toast.error("Please upload an invoice photo.");
      return;
    }
    ...
    try {
      ...
      if (!items.length) {
        toast.error("No valid items extracted from this image.");
        return;
      }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success(result?.message || "Image deliveries saved successfully.");
  ...
  return (
    <div className="relative p-6 md:p-8 max-w-7xl mx-auto space-y-6 overflow-hidden">
      <div className="pointer-events-none absolute -top-28 -right-20 h-72 w-72 rounded-full bg-amber-200/40 blur-3xl dark:bg-amber-500/10" />
      ...
      <section className="relative rounded-2xl border border-slate-200 bg-white/95 p-5 md:p-6 shadow-sm dark:border-white/10 dark:bg-[#171B24]/95 space-y-3">
        <h2 className="text-lg font-bold text-slate-900 dark:text-slate-100 inline-flex items-center gap-2">
      ...
      <section className="relative rounded-2xl border border-slate-200 bg-white/95 p-5 md:p-6 shadow-sm dark:border-white/10 dark:bg-[#171B24]/95 space-y-4">
        ...
        {draftItems.length === 0 ? (
          <div className="rounded-xl border border-dashed border-slate-300 p-8 text-center text-sm text-slate-500 dark:border-white/15 dark:text-slate-400">
            Upload image and run OCR to preview draft items.
          </div>
        ) : (
          <>
                    ...
                    <tr key={item.draftId} className="border-t border-slate-100 dark:border-white/10">
                      ...
                      <td className="p-2.5">
                        {item.isExistingProduct ? (
                          ...
                          <span className="inline-flex rounded-full bg-amber-100 px-2 py-1 text-xs font-semibold text-amber-700 dark:bg-amber-500/20 dark:text-amber-300">
                            New Product
                          </span>
                        )}
                      </td>
