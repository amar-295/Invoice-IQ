...
const exportCSV = (data: ReportData, preset: PresetKey) => {
  ...
  lines.push(
    `${data.summary.totalSpend},${data.summary.deliveryCount},${data.summary.uniqueSellers},${data.summary.uniqueProducts}`,
  );
  lines.push("");
  ...
  data.supplierBreakdown.forEach((r) =>
    lines.push(
      `"${r.name}",${r.totalSpend},${r.deliveryCount},${r.avgOrderValue},${r.percentOfTotal}`,
    ),
  );
  lines.push("");
  ...
  data.productBreakdown.forEach((r) =>
    lines.push(
      `"${r.name}","${r.unit}",${r.avgPrice},${r.minPrice},${r.maxPrice},${r.totalQty},${r.deliveryCount}`,
    ),
  );
  lines.push("");
  ...
  data.deliveryLog.forEach((r) =>
    lines.push(
      `"${fmtDate(r.date)}","${r.sellerName}","${r.productName}",${r.quantity},"${r.unit}",${r.price},"${r.source}"`,
    ),
  );
                  ...
                  data.supplierBreakdown.map((row, i) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-right font-semibold text-gray-900 dark:text-white">
                        {fmtRupee(row.totalSpend)}
                      </td>
                      <td className="px-4 py-3 text-right text-gray-600 dark:text-gray-300">
                        {row.deliveryCount}
                      </td>
                      <td className="px-4 py-3 text-right text-gray-600 dark:text-gray-300">
                        {fmtRupee(row.avgOrderValue)}
                      </td>
              ...
              <tbody>
                {loading ? (
                  ...
                  data.productBreakdown.map((row) => {
                    ...
                    const spreadPct =
                      row.maxPrice > 0
                        ? ((spread / row.maxPrice) * 100).toFixed(1)
                        : "0.0";
                    return (
                      <tr
                        key={row.name}
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-right font-semibold text-gray-900 dark:text-white">
                          {fmtRupee(row.avgPrice)}
                        </td>
                        <td className="px-4 py-3 text-right text-emerald-600 dark:text-emerald-400">
                          {fmtRupee(row.minPrice)}
                        </td>
